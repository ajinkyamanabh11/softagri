...
class _AppLockScreenState extends State<AppLockScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _attemptAuthentication() async {
    ...
    try {
      ...
      if (authenticated) {
        // Check if we have necessary data loaded
        final httpService = Get.find<HttpDataServices>();
        if (httpService.subfolderRx.value.isEmpty) {
          // Navigate to data loading first
  ...
  void _handleAuthenticationError(dynamic error) {
    ...
    Get.snackbar(
      ...
      'An error occurred during authentication: ${error.toString()}',
